...
export async function register(ctx: any) {
    ...
    const result = await client.execute('INSERT INTO users (name, email, password) VALUES (?, ?, ?)', [name, email, password]);
...
export async function login(ctx: any) {
    ...
    const result = await client.execute('SELECT * FROM users WHERE email = ?', [body.value.email]);
